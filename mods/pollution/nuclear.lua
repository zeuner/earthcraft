...
minetest.register_tool("pollution:nukers", {
	...
	inventory_image = "pollution_log3.png",
		on_place = function(itemstack, user, pointed_thing)
			...
			if np~=nil and minetest.get_item_group(minetest.get_node(np).name, "stone")==0 then
...
on_timer = function (pos, elapsed)
		...
		if meta:get_int("power")==0 then
			...
			for i,np in pairs(pollution.getwired(pos,"pollution:ngen")) do
				...
				minetest.get_node_timer(np):start(1)
...
minetest.register_node("pollution:ngen", {
	...
	tiles = {"default_bronze_block.png^carts_rail_crossing.png","default_bronze_block.png^default_ladder_steel.png"},
	is_ground_content = false,
	groups = {cracky = 1,stone = 1},
	sounds = default.node_sound_stone_defaults(),
