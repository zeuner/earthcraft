...
local pollution_mob=function(self, dtime)
	...
	if self.timer2<1 then return self end
		self.timer2=0
-- if attacking
...
--fron of a wall
		...
		local junode=minetest.registered_nodes[minetest.get_node({x=jnpos.x,y=jnpos.y-1,z=jnpos.z}).name]
		...
		if junode and junode.walkable then
			...
			if (nodes[3]==true and nodes[4]==false) then -- jump
				...
				self.object:setvelocity({x = self.movment.x, y = 5, z =self.movment.z})

				minetest.after(0.5, function(self)
					self.object:setvelocity({x = self.movment.x, y = self.object:getvelocity().y,z = self.movment.z})
				end,self)
...
--falling
	...
	if self.timer<2 then return self end
--other
	self.timer=0
	if self.object==nil then return self end

	local pos=self.object:getpos()
	local node=minetest.get_node(pos).name
	if minetest.get_item_group(node, "water")>0 or minetest.get_item_group(node, "lava")>0 then
...
function pollution_spawn_kmob(pos,special,nodrop)
	...
	if minetest.get_item_group(node.name, "unbreakable")>0 or (
	minetest.get_item_group(node.name, "fleshy")==0 and
	minetest.get_item_group(node.name, "choppy")==0 and
	minetest.get_item_group(node.name, "bendy")==0 and
	minetest.get_item_group(node.name, "cracky")==0 and
	minetest.get_item_group(node.name, "crumbly")==0 and
	minetest.get_item_group(node.name, "snappy")==0)
	then return false end

	minetest.set_node(pos, {name = "air"})
	pollution_km_tmps.con=1
	pollution_km_tmps.node_name=node_name
...
--special
	...
	if minetest.get_item_group(node.name, "wood")>0 then pollution_km_tmps.team="wood" pollution_km_tmps.hp=10 pollution_km_tmps.sound="default_wood_footstep" end
	if minetest.get_item_group(node.name, "stone")>0 then pollution_km_tmps.team="stone" pollution_km_tmps.hp=15 pollution_km_tmps.sound="default_dig_cracky" end
	if minetest.get_item_group(node.name, "soil")>0 then pollution_km_tmps.team="dirt" pollution_km_tmps.hp=5 pollution_km_tmps.sound="default_dig_choppy" end
	if minetest.get_item_group(node.name, "flora")>0 then pollution_km_tmps.team="flora" pollution_km_tmps.hp=5 pollution_km_tmps.sound="default_dirt_footstep" end
	if minetest.get_item_group(node.name, "snappy")>0 then pollution_km_tmps.team="flora" pollution_km_tmps.hp=5 pollution_km_tmps.sound="default_dirt_footstep" end
	if minetest.get_item_group(node.name, "tree")>0 then pollution_km_tmps.team="wood" pollution_km_tmps.hp=15 pollution_km_tmps.sound="default_wood_footstep" end
	if minetest.get_item_group(node.name, "barrel")>0 then pollution_km_tmps.team="barrel" pollution_km_tmps.hp=50 pollution_km_tmps.sound="pollution_punsh" pollution_km_tmps.barrel=minetest.get_item_group(node.name, "barrel") end
	...
	if minetest.get_item_group(node.name, "barrel")>0 or node.name:find("pollution:bottle",1) or node.name:find("pollution:crystal_",1) then
...
local function pollution_throw(user,type)
	...
	local m=minetest.add_entity(upos, "pollution:bottle1")
...
on_activate=function(self, staticdata)
		...
		if pollution_km_tmps.con then
			...
			self.node_name=pollution_km_tmps.node_name
			if pollution_km_tmps.type~="normal" then self.atta=-1 end
			pollution_km_tmps={}
			self.new=true
		else
			local pos=self.object:getpos()
			pollution_spawn_kmob(pos,0,0)
			self.object:remove()
			return self
		end
		self.hp=self.object:get_hp()
		self.movment={x=0,y=0,z=0}
...
get_staticdata = function(self)
		if self.new then
			self.new=nil
		elseif self.object and self.node_name then
			local pos=self.object:getpos()
			local n=minetest.get_node(pos).name
			if minetest.registered_nodes[n] and minetest.registered_nodes[n].buildable_to then
				minetest.set_node(pos,{name=self.node_name})
			end
		end
	end,
