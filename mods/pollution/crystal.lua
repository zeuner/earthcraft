...
--pollution:crystal_soil
	...
	for i=0,rnd,1 do
		...
		if minetest.get_item_group(n.name, "tree")>0 then minetest.set_node(np, {name = "pollution:crystal_block"}) end
		if minetest.get_item_group(n.name, "snappy")>0 then
			...
			if math.random(200)==1 and minetest.get_item_group(minetest.get_node({x=np.x,y=np.y-1,z=np.z}).name, "soil")>0 then
