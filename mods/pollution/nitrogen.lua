...
--1=player 2=enitity
	...
	-- ================= if an item/mob
	if object==2 then
	if not (ob and ob:get_luaentity()) or ob:get_luaentity().name=="pollution:icicle" then return true end
	if ob:get_hp()<=4 or kill==1 then
		local m=minetest.add_entity(pos, "pollution:icicle")
		m:get_luaentity().crash=true
		local p=ob:get_properties()
		if p.visual=="mesh" and p.mesh~="" and p.mesh~=nil then
		m:setyaw(ob:getyaw())
		m:set_properties({
			visual_size=p.visual_size,
			visual="mesh",
			mesh=p.mesh,
			textures={"default_ice.png^pollution_blue2.png","default_ice.png^pollution_blue2.png","default_ice.png^pollution_blue2.png","default_ice.png^pollution_blue2.png","default_ice.png^pollution_blue2.png"},
...
minetest.register_abm({
	...
	action = function(pos,node)
		local name=minetest.get_node(pos).name
		if minetest.get_item_group(name, "liquid")>0  or name=="pollution:water_source" or name=="pollution:water_flowing" then
		...
		if minetest.get_item_group(name, "leaves")==1 then
		...
		if name=="pollution:frozen_plant" then
			for i=1,100,1 do
				if minetest.get_item_group(minetest.get_node(pos).name, "snappy")>0 then
		...
		if minetest.get_item_group(name, "flora")==1 or minetest.get_item_group(name, "snappy")>=1 then
