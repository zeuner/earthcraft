...
minetest.register_abm({
	...
	action = function(pos)
		...
		if minetest.get_node(pos).name=="air" then
			if name=="pollution:ice" and math.random(50)==1 then minetest.add_entity(pos, "pollution:axov_ice") end
			if name=="pollution:crystal_soil" and math.random(50)==1 then minetest.add_entity(pos, "pollution:axov_crystal") end
			if name=="pollution:dirt" and math.random(50)==1 then minetest.add_entity(pos, "pollution:axov") end
			if name=="pollution:acid_fire" and math.random(30)==1 then minetest.add_entity(pos, "pollution:axovy") end
			if name=="pollution:n_fire" and math.random(30)==1 then minetest.add_entity(pos, "pollution:axovu") end
		end
	end,
})
		...
		on_use = function(itemstack, user, pointed_thing)
			if pointed_thing.type=="node" then
				if pollution_mobs_max(0,1) then
					...
					minetest.add_entity(p, "pollution:axov")
				else
		...
		on_use = function(itemstack, user, pointed_thing)
			if pointed_thing.type=="node" then
				if pollution_mobs_max(0,1) then
					local p=pointed_thing.above
					p={x=p.x,y=p.y+0.5,z=p.z}
					minetest.add_entity(p, "pollution:axov_ice")
				else
					minetest.chat_send_player(user:get_player_name(), "Too many pollution axov mobs: (max " .. pollution_mobs_axov_max_number .. ")")
				end
			end
		...
		on_use = function(itemstack, user, pointed_thing)
			if pointed_thing.type=="node" then
				if pollution_mobs_max(0,1) then
					local p=pointed_thing.above
					p={x=p.x,y=p.y+0.5,z=p.z}
					minetest.add_entity(p, "pollution:axov_crystal")
				else
					minetest.chat_send_player(user:get_player_name(), "Too many pollution axov mobs: (max " .. pollution_mobs_axov_max_number .. ")")
				end
			end
		...
		on_use = function(itemstack, user, pointed_thing)
			if pointed_thing.type=="node" then
				if pollution_mobs_max(0,1) then
					local p=pointed_thing.above
					p={x=p.x,y=p.y+0.5,z=p.z}
					minetest.add_entity(p, "pollution:axovy")
				else
					minetest.chat_send_player(user:get_player_name(), "Too many pollution axov mobs: (max " .. pollution_mobs_axov_max_number .. ")")
				end
			end
		...
		on_use = function(itemstack, user, pointed_thing)
			if pointed_thing.type=="node" then
				if pollution_mobs_max(0,1) then
					local p=pointed_thing.above
					p={x=p.x,y=p.y+0.5,z=p.z}
					minetest.add_entity(p, "pollution:axovu")
				else
					minetest.chat_send_player(user:get_player_name(), "Too many pollution axov mobs: (max " .. pollution_mobs_axov_max_number .. ")")
				end
			end
...
-- dig a node
	if remove and remove==2 then
		...
		if minetest.is_protected(ob,"") or self.toxic==0 then
			...
			if minetest.get_item_group(name, "unbreakable")>0 or minetest.get_node(ob).drop=="" or name=="default:chest_locked" then return false end
...
--hurting
		if tpos<=3 and math.random(2)==1 then
			self.object:setvelocity({x=0, y=self.move.y, z=0})
			self.status_target1:set_hp(self.status_target1:get_hp()-self.dmg)
			self.status_target1:punch(self.object, {full_punch_interval=1.0,damage_groups={fleshy=7}}, "default:bronze_pick", nil)
			...
			if self.status_target1:get_hp()<=0 and self.status_target1:is_player()==true then
				...
				if t~=nil then
					...
					self.object:set_properties({mesh = "character.b3d",textures=t})
				end
			end
		end
	end
...
on_activate=function(self, staticdata)
		...
		self.move={x=0,y=0,z=0,jump=0,jump_timer=0,speed=2}
	end,
...
function pollution_throw2(self)
	...
	if self.toxic==1 then
		...
		if item~="pollution:bottle" and item~="pollution:bottlepur" then
			...
			m=minetest.add_entity({x=pos.x+(self.move.x*3),y=pos.y+1,z=pos.z+(self.move.z*3)}, "pollution:bottle1")
		...
		m=minetest.add_entity({x=pos.x+(self.move.x*3),y=pos.y+1,z=pos.z+(self.move.z*3)}, "pollution:icicle")
